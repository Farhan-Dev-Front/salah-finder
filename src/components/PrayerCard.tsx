...
const Dropdown = ({ current, onSelect, dark }: DropdownProps) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <span>
          {AZAN_PRESETS.find((p) => p.id === current)?.label || "Select"}
        </span>
        <svg
...
const PrayerCard = ({
  ...
  const currentAudio = azan.perPrayerAudio
    ? azan.perPrayerAudio[name]
    : azan.audioId;

  const handleSelect = async (id: string) => {
    setAzanSettings({
      perPrayerAudio: { ...(azan.perPrayerAudio || {}), [name]: id },
    });
  ...
  const [playing, setPlaying] = useState<{ ownerId: string | null; audioId: string | null; prayer?: string | null }>(
    { ownerId: getPlayingId(), audioId: null, prayer: null }
  );
  useEffect(() => subscribePlaying((v) => setPlaying(v)), []);
  const [myOwner, setMyOwner] = useState<string | null>(null);

  const handlePlaySample = async () => {
    if ((playing.ownerId !== null && playing.ownerId === myOwner) || playing.prayer === name) {
      stopAzan();
      return;
    }
    if (busy) return;
  ...
  const toggleLocalPerPrayer = () => {
    const current = azan.perPrayer?.[name];
    // If parent supplied a handler, prefer it (avoid double-toggle).
    if (onToggleSound) {
      onToggleSound();
      return;
    }
    setAzanSettings({ perPrayer: { ...(azan.perPrayer || {}), [name]: !current } });
  };

  return (
    <div
      className={`p-4 rounded-xl shadow-md ${active ? "bg-gradient-to-br from-purple-600 to-indigo-600 text-white" : "bg-white text-gray-800"}`}
    >
      ...
      <div className="mt-3 flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          {active && !isCurrent && (
            <div className="ml-2 text-xs px-2 py-1 rounded bg-white/10">
              Next
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 relative">
          ...
          <button
            ...
            disabled={busy && playing.ownerId !== myOwner && playing.prayer !== name}
            className="text-sm px-3 py-1 rounded bg-purple-600 text-white disabled:opacity-60"
          >
            {(playing.ownerId !== null && playing.ownerId === myOwner) || playing.prayer === name
              ? "Stop"
              : busy
                ? "Playing..."
                : "Play"}
          ...
          <button
            onClick={toggleLocalPerPrayer}
            className={`text-sm px-2 py-1 rounded ${soundOn ? "bg-white/10" : "bg-gray-100 text-gray-700"}`}
            aria-label={soundOn ? `Disable azan for ${name}` : `Enable azan for ${name}`}
          >
            {soundOn ? "🔊" : "🔇"}
          </button>
        </div>
